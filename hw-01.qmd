...
plot_3<-ggplot() +
  ...
  geom_text(data = cities, aes(x = long, y = lat, label = city), vjust = 1.5) +#add text to city points which below the points
  scale_size(range = c(0.5, 15)) + 
...
```{r}
temp_plot_1<-ggplot(temperatures, aes(long, temp)) +
    geom_path(color="grey", size=1.5) +
    geom_point(size=2) 
temp_plot_1

```
Step 5) Add lables to the temperatures's datase 
...
temp_plot_2<-ggplot(temperatures, aes(long, temp)) +
    geom_path(color="grey", size=1.5) +
    geom_point(size=1) +
  geom_text(aes(label=nice.label), size=2, vjust=1.5)
temp_plot_2
